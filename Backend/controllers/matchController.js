...
const getMatches = asyncHandler(async (req, res) => {
	...
	const matches = await User.find({
		...
		.select(
			"_id username fullName anonymousAlias avatarEmoji bio interests gender"
		)
		.lean();

	// Sort by most interests in common, then by recent
	const sorted = matches.sort((a, b) => {
		const overlapA = a.interests.filter((i) =>
			currentUser.interests.includes(i)
		).length;
		const overlapB = b.interests.filter((i) =>
			currentUser.interests.includes(i)
		).length;
	...
	// Pagination logic
	const isPremium =
		currentUser.premiumMatchUnlocks && currentUser.premiumMatchUnlocks > 0;
...
const unlockPremiumMatches = asyncHandler(async (req, res) => {
	...
	res.json({
		message: "Premium matches unlocked!",
		premiumMatchUnlocks: user.premiumMatchUnlocks,
	});
});
